/*
 * sim_st7789.c - PC simulator backend for the NV3007 driver API.
 *
 * Implements the same ST7789_* API surface as HAL/NV3007.c (which is
...
{
    if (g_cc <= ST7789_VIS_X1 && g_rc <= 427) {
        uint16_t lx = g_rc;                       /* logical x = page */
        uint16_t ly = (uint16_t)(ST7789_VIS_X1 - g_cc); /* logical y = 153 - column */
        if (lx < 428 && ly < 142) {
...
void ST7789_SetFontZoom(uint8_t zoom) { (void)zoom; }
