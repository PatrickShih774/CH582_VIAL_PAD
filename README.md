...
- **现状（B0.8.10）**：
  ...
  - 🏆 **深睡电流优化根因链（里程碑：460µA → 34µA）**：
    ...
    |------|------------|
    | 80mA | 屏幕亮（背光 + TFT 常开） |
    | 330µA | ~~连接态 BLE 保活物理下限~~ **已推翻（B0.8.11）**：iOS 实际接受从机请求的间隔，15ms 只是当时请求值即 15ms（见下） |
    | **~75µA** | **连接态（800ms 间隔，B0.8.11 实测）**：请求 `800ms` iOS 即接受（回包 `iv=648`≈810ms / `lt=0`），平均 ≈ 70µA 睡眠地板 + 800µA×~5ms/810ms ≈ **75µA** |
    | 80µA | **广播态（未连接）**：保持广播（间隔 1s）+ 深睡的平均，BLE 栈 RTC 定时唤醒广播 |
    ...
    **关键根因总结**：
    1. ~~连接态 330µA 是 BLE 保活物理上限（iOS 强制短间隔）~~ **已推翻（B0.8.11）**：iOS 接受从机请求的连接间隔（实测 800ms 被接受，`iv=648`），连接态平均电流可降到 ~75µA；深睡（关 BLE）仍是更低档 34µA。
  ...
  - 🏆 **B0.8.11 里程碑：iOS 连接间隔可拉长（推翻「强制 15ms」）**：
...
| RTC | 时钟显示 | 用外部 32.768k 晶振 LSE（`CLK_OSC32K=0`），精度 ±20ppm、深睡 0.08mA 更省电，已加起振等待修复卡死 |
