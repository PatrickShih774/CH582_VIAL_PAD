...
#### 5.8.1 屏幕驱动 ✅ 已完成（CS 接地 + 横向 + 3× 字体 + 防花屏）
...
- **字体**：5×7 原始数据 + 3× 缩放（15×24），支持居中显示
- **已实现**：init、全屏填充、矩形填充、画点、3× 缩放字符/字符串、水平/垂直线、背光控制
- **待办**：TMR2 PWM 调光
...
### 5.9 自定义上位机计划（2026-08-02 规划）

标准 VIAL 无法实现的屏幕功能（自定义显示字符、RTC 校时等），通过**自定义上位机 + raw HID** 实现。

#### 5.9.1 通信机制

- **复用 USB raw HID**（EP3，usage page `0xFF60`，VIAL 同一通道）
- 自定义命令协议：`0xFE` 前缀区分 VIAL 命令与自定义命令（避免冲突）
- 固件端在 `USB_MODE.c` `DevEP3_OUT_Deal` 增加自定义命令处理

#### 5.9.2 功能清单（规划）

| 功能 | 说明 | 状态 |
|------|------|------|
| **RTC 校时** | 上位机发送日期时间 → 固件 `RTC_InitTime()` 写入，校准 RTC | 规划 |
| **屏幕自定义字符** | 上位机发送文字 → 固件存 EEPROM → 屏幕首页显示 | 规划 |
| **屏幕亮度调节** | TMR2 PWM 背光控制（0-255） | 规划 |
| **固件配置读写** | 模式切换（USB/BLE/2.4G）、参数读写 | 规划 |
| **诊断信息回读** | RTC 时间、固件版本、模式状态等回传上位机 | 规划 |

#### 5.9.3 协议草案

```
校时命令:    FE E1 [年2B][月][日][时][分][秒]     → 固件写 RTC
设屏文字:    FE E2 [len][ASCII 文字...]          → 固件存 EEPROM 并显示
读屏文字:    FE E3                              → 固件回读 EEPROM 文字
背光:        FE E4 [level 0-255]                → TMR2 PWM
诊断:        FE E5                              → 回传时间/模式/版本
```

#### 5.9.4 待实现

- **固件端**：raw HID 校时命令、屏幕字符存储（EEPROM）、背光 PWM、诊断回读
- **上位机**：Python/网页工具，枚举 raw HID 设备，实现校时/设文字/调亮度
- **文档**：协议定义、上位机使用说明
