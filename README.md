...
| 项 | 状态 |
|----|------|
| 版本 | **B0.8.10**（低功耗：HAL_SLEEP/DCDC + 待机关屏/SPI引脚高阻 + 外部 32K 晶振 LSE + 秒级测试档；**深睡 34µA 里程碑**；连接态 330µA / 广播态 80µA（保持连接的 BLE 保活物理上限）） |
...
- **现状（B0.8.10）**：
  ...
  - 🏆 **深睡电流优化根因链（里程碑：460µA → 34µA）**：
    ...
    |------|------------|
    | 80mA | 屏幕亮（背光 + TFT 常开） |
    | 330µA | **连接态 BLE 保活**：iOS HID 主机强制 ~15ms 短连接间隔，从机请求 1s 长间隔被拒绝，属物理下限，软件无解 |
    ...
    **关键根因总结**：
    1. 连接态 330µA 是 BLE 保活物理上限（iOS 强制短间隔），无法软件突破；深睡必须先关 BLE。
