...
- 连接参数请求本身**违反 Apple 6 规则**（`1s×17=17s` 超 Apple `interval×(lat+1)≤2s` 上限），iOS/协议栈拒绝的是**违规参数集**，不是"1s 太长"。
- 参数更新结果**从未被代码捕获**，"iOS 拒绝"是从电流反推的猜测。
- DCDC 在源码中**从未被开启**，所有电流数字都在 LDO bypass 下测得。

**因此 330µA 不是已证实的物理下限，而是"从未成功协商参数 + DCDC 没开 + 定时器空转"的叠加嫌疑值。** 且 v3 新增一个根本约束：即使 A1 修好，iOS 受 Apple 规则 3（`interval×(lat+1)≤2s`）+ 规则 5（`lat≤30`）+ iOS 倾向选 min 间隔三重夹击，**连接态保活 sleep window 在 iOS 上有上限**（见 §4/§10 推导）。故"始终连接能否省过深睡"不取决于参数技巧，而取决于 **D3+D4 能把 EXTEND-on 地板压到多低** + **EXTEND 域决策⑥**。

---

## §1 现状基线（嫌疑标注）

| 测量值 | README 定性 | v3 定性 | 嫌疑原因 |
|---|---|---|---|
| 连接态 330µA | 物理下限、软件无解 | **未证实，待推翻** | A1 违规致协商从未成功；A3 DCDC 关；A4/A5 定时器空转 |
...
### 🔴 A1｜连接参数请求违反 Apple 规则--iOS 必然拒绝（主因）
...
**校验用 Apple Accessory Design Guidelines 6 规则（QA1931）**：
1. `Interval Min ≥ 15ms` 且为 15ms 整数倍
2. `Interval Min + 15ms ≤ Interval Max`
3. `Interval Max × (Slave Latency + 1) ≤ 2s`
4. `Interval Max × (Slave Latency + 1) × 3 < connSupervisionTimeout`
5. `Slave Latency ≤ 30`
6. `2s ≤ connSupervisionTimeout ≤ 6s`

代入 C-base：规则 3 `1s×17=17s > 2s` ✗；规则 4 `1s×17×3=51s > 5s` ✗ -> **iOS 必拒**。

**定性**：iOS 拒的不是"1s 太长"，而是 **`1s×17=17s` 超 Apple 规则 3 的 2s 上限**。一个 `1s×1=1s≤2s` 的合规请求（C2）理论可过。这直接证伪 README"1s 物理下限"。
...
- `LowPower_Sleep`（`CH58x_pwr.c:312`）只 `R16_POWER_PLAN & (RB_PWR_DCDC_EN|RB_PWR_DCDC_PRE)` **保持**既有位，**从不把关->开**。
...
`BLE_MODE.c:101-107` `scanRspData` 用 `DEFAULT_DESIRED_*`=1s/1s，主机扫描响应阶段即见 1s/1s。
...
## §4 参数候选表（Apple 6 规则重写）

| 候选 | min-max(单位) | interval | lat | timeout | Apple 校验 | 用途/预期 |
|---|---|---|---|---|---|---|
| C-base(现) | 800-800 | 1s | 16 | 5s | r3 17s>2s ✗ r4 51s>5s ✗ | 证明现状违规 |
| **C1′** | 12-24 (15-30ms) | iOS 选 15ms | 29 | 5s(500) | 全过 ✓（max=30 过 r2） | A 组：iOS 选 min -> sleep window **450ms**，延迟优、不省电 |
| **C2** | 800-800 | 1s | 0 | 6s(600) | r3 1s≤2s ✓ r4 3s<6s ✓ r6 6∈[2,6] ✓ | B 组：iOS 是否**接受**强制长间隔 -> window 1s |
| **C2+** | 1584-1596 | ~1.98s | 0 | 6s(600) | r3 1.98s≤2s ✓ r4 5.94s<6s ✓ | B 组上限：若 iOS 接受强制长间隔，window 可近 2s |
| C1′max | 12-24 | iOS 选 15ms | 30 | 6s | r3 30×31=930ms≤2s ✓ r4 2.79s<6s ✓ | 高延迟极限：window 465ms |

**关键推导（决定 §6⑤ 与 §10）**：
- iOS 倾向选 min 间隔 -> 带 15ms 的请求实际协商成 **15ms**。
- 受 Apple r3(`≤2s`)+r5(`lat≤30`) -> **高延迟路线 window 上限 = 15ms×31 = 465ms**。
- 而**强制长间隔路线**（min=max=长，C2/C2+）若 iOS 接受 -> window 可达 **~2s**（r3 上限）。
...
### D2（A+B 组）
- A 组：发 **C1′**（15-30ms, lat29, 5s）-> 抓 iOS 实际选的 interval（预期 15ms，window 450ms）。
- B 组：发 **C2**（1s, lat0, 6s）-> **iOS 是否接受强制长间隔**（若接受 window 1s；若拒看 iOS 给什么）。再试 **C2+**（~2s）探上限。
...
| E2 | 是(iOS) | C1′ 合规 | 关 | 运行 | 关 | **A 组：iOS 是否接受 Latency** |
...
**⑤ 深睡取舍（重写）**：
- v2 误把"始终连接 ~35-45µA"归功于 C1 省电。**v3 修正**：iOS 上 A 组 window 仅 450-465ms、且不省电；**始终连接能否省过深睡，取决于 D3+D4 把 EXTEND-on 地板压到多低 + EXTEND 决策⑥，与 C1 无关**。
...
D0 读DCDC位+VDCID/VDCIA硬件+ROM bit13 ; D1 仪表化(T1/T2/T3/T5 RAM)
        │ 读真实协商值 ─ 证伪 A1
        │
   D2 A组(C1′) ─ iOS 选 15ms -> window 450ms（延迟优/不省电，证 #2）
   D2 B组(C2/C2+) ─ iOS 是否接受强制长间隔？
        │
   ┌────┴───────────────────┐
...
| 1 | D2 用 **C1′(15/30ms,lat29,5s)** + B 组 **C2(1s,lat0,6s)/C2+**，抓 iOS 实际选值 | 候选按 Apple 6 规则；B 组定胜负 |
...
> **设计基线**：所有模式参数预先通过 Apple 6 规则校验。iOS 会选请求的 min 间隔，故"实际间隔 = 请求 min"。在 r2(`min+15≤max`)+r3(`max×(lat+1)≤2s`)+r5(`lat≤30`) 三重夹击下：
...
间隔单位 1.25ms，超时单位 10ms。所有请求均已过 Apple 6 规则校验。

| 模式 | 请求 min-max(lat,timeout) | iOS 实选 | sleep window | 输入延迟 | TX 功率 | 屏/背光 | 扫描节拍 | 目标电流(校准目标) |
|---|---|---|---|---|---|---|---|---|
| **M1 竞速** | 12-24 (15-30ms, lat0, 2s) | 15ms | 15ms | 15ms | 0dBm | ON | 挂连接事件(15ms) | ~300-500µA |
| **M2 办公** | 12-24 (15-30ms, lat29, 5s) | 15ms | 450ms | 15ms | 0/-3dBm | ON(可dim) | 挂连接事件 | ~50-100µA* |
...
**模式间 window 单调递增**：15ms(M1) -> 450ms(M2, 30×) -> 1395ms(M3, 3×) -> 1980ms(M4, 1.4×)。
> 注意 M3->M4 window 增益小(1.4×)但延迟代价大(45ms->2s)，**M4 的价值在于"接近 2s 连接态上限"**；若 EXTEND-on 地板压不到 ~15µA，M4 连接态达不到 <20µA，此时 M4 应回退为 **BLE-off 断连深睡(34µA)**（见 10.3）。

### 10.2 状态机与切换逻辑

```
                 任意按键 ─────────────────────► M1 竞速(15ms,lat0)
                    ▲                               │
                    │                             3s 无按键
            (有键即升,目标=M1)                       ▼
                    │                            M2 办公(15ms,lat29)
