...
static const uint8_t font_5x7[][5] = {
    ...
    {0x3C,0x42,0x42,0x7E,0x3C}, /* 0x30 0 */
    {0x00,0x22,0x7E,0x02,0x00}, /* 0x31 1 */
    {0x42,0x46,0x4A,0x52,0x62}, /* 0x32 2 */
    {0x46,0x42,0x52,0x52,0x6E}, /* 0x33 3 */
    {0x08,0x18,0x28,0x7E,0x08}, /* 0x34 4 */
    {0x7A,0x4A,0x4A,0x4A,0x4E}, /* 0x35 5 */
    {0x1C,0x32,0x52,0x5E,0x0C}, /* 0x36 6 */
    {0x40,0x46,0x48,0x50,0x60}, /* 0x37 7 */
    {0x34,0x4A,0x4A,0x7E,0x34}, /* 0x38 8 */
    {0x30,0x49,0x49,0x79,0x3E}, /* 0x39 9 */
    ...
    {0x1E,0x30,0x50,0x30,0x1E}, /* 0x41 A */
    {0x7E,0x52,0x52,0x52,0x2C}, /* 0x42 B */
    {0x3C,0x42,0x42,0x42,0x24}, /* 0x43 C */
    {0x7E,0x42,0x42,0x42,0x3C}, /* 0x44 D */
    {0x7E,0x52,0x52,0x52,0x42}, /* 0x45 E */
    {0x7E,0x50,0x50,0x50,0x40}, /* 0x46 F */
    {0x3C,0x42,0x42,0x4A,0x2E}, /* 0x47 G */
    {0x7E,0x10,0x10,0x10,0x7E}, /* 0x48 H */
    {0x00,0x42,0x7E,0x42,0x00}, /* 0x49 I */
    {0x00,0x04,0x42,0x42,0x7C}, /* 0x4A J */
    {0x7E,0x10,0x10,0x28,0x46}, /* 0x4B K */
    {0x7E,0x02,0x02,0x02,0x02}, /* 0x4C L */
    {0x7E,0x20,0x18,0x20,0x7E}, /* 0x4D M */
    {0x7E,0x20,0x10,0x08,0x7E}, /* 0x4E N */
    {0x3C,0x42,0x42,0x42,0x3C}, /* 0x4F O */
    {0x7E,0x48,0x48,0x48,0x30}, /* 0x50 P */
    {0x3C,0x42,0x4A,0x44,0x3A}, /* 0x51 Q */
    {0x7E,0x48,0x48,0x4C,0x32}, /* 0x52 R */
    {0x22,0x52,0x52,0x52,0x4C}, /* 0x53 S */
    {0x40,0x40,0x7E,0x40,0x40}, /* 0x54 T */
    {0x7C,0x02,0x02,0x02,0x7C}, /* 0x55 U */
    {0x78,0x04,0x02,0x04,0x78}, /* 0x56 V */
    {0x7E,0x04,0x18,0x04,0x7E}, /* 0x57 W */
    {0x46,0x28,0x10,0x28,0x46}, /* 0x58 X */
    {0x40,0x20,0x1E,0x20,0x40}, /* 0x59 Y */
    {0x42,0x46,0x4A,0x52,0x62}, /* 0x5A Z */
    ...
    {0x16,0x28,0x28,0x28,0x1E}, /* 0x61 a */
    {0x7E,0x12,0x12,0x12,0x0C}, /* 0x62 b */
    {0x1C,0x22,0x22,0x22,0x14}, /* 0x63 c */
    {0x0C,0x12,0x12,0x12,0x7E}, /* 0x64 d */
    {0x1C,0x2A,0x2A,0x2A,0x18}, /* 0x65 e */
    {0x10,0x3E,0x50,0x40,0x00}, /* 0x66 f */
    {0x12,0x29,0x29,0x29,0x3E}, /* 0x67 g */
    {0x7E,0x10,0x10,0x10,0x0E}, /* 0x68 h */
    {0x00,0x02,0x5E,0x12,0x00}, /* 0x69 i */
    {0x00,0x02,0x01,0x11,0x5E}, /* 0x6A j */
    {0x7E,0x08,0x08,0x14,0x02}, /* 0x6B k */
    {0x00,0x42,0x7E,0x02,0x00}, /* 0x6C l */
    ...
    {0x10,0x7C,0x12,0x02,0x00}, /* 0x74 t */
    {0x3C,0x02,0x02,0x02,0x3E}, /* 0x75 u */
    {0x38,0x04,0x02,0x04,0x38}, /* 0x76 v */
    {0x3E,0x04,0x18,0x04,0x3E}, /* 0x77 w */
...
void ST7789_DrawChar(char ch, uint16_t x, uint16_t y, uint16_t color, uint16_t bg)
{
     ...
     * ST7789 with MADCTL MY=1 scans RAMWR bottom-up, so rows are drawn
     * in reverse order (bit0 first → appears at bottom; bit7 last → top).
     * Each font pixel is rendered as a z×z block. */
    for (j = 0; j < 8; j++) {
        uint8_t row = 7 - j;   /* invert row for bottom-up RAMWR scan */
        for (py = 0; py < z; py++) {
            for (i = 0; i < 5; i++) {
                uint8_t on = (font_5x7[idx][i] & (0x80 >> row)) ? 1 : 0;
                for (px = 0; px < z; px++) {
                    if (on) {
                        SPI_WriteByte((uint8_t)(color >> 8));
                        SPI_WriteByte((uint8_t)(color & 0xFF));
